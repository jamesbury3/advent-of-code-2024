...
type Day8Part2Solver struct{}

func (solver *Day8Part2Solver) Solve(lines []string) (string, error) {
	...
	for _, locations := range antennas {
		...
		for _, location := range locations {
			for _, otherLocation := range locations {
				...
				destinationRow := location[0]
				destinationCol := location[1]

				for isDestinationWithinGrid(destinationRow, destinationCol, height, width) {
					destinationRow = destinationRow + xDist
					destinationCol = destinationCol + yDist
...
func isDestinationWithinGrid(destinationRow, destinationCol int, height, width int) bool {
	return destinationRow >= 0 && destinationRow < height &&
		destinationCol >= 0 && destinationCol < width
}
