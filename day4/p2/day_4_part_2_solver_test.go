...
func TestDay4Part2Solver_Solve_InputExample(t *testing.T) {
	type fields struct {
		daySolverDelegate *Day4Part2Solver
	}
	tests := []struct {
		name    string
		fields  fields
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("Day4Part2Solver.Solve() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("Day4Part2Solver.Solve() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestDay4Part2Solver_Solve_Input(t *testing.T) {
	type fields struct {
		daySolverDelegate *Day4Part2Solver
	}
	tests := []struct {
